...
export default function Navbar() {
    ...
    const [transparent, setTransparent] = useState(location.pathname === LINKS_DATA.main.to)
    
    const toggleNavbar = () => {
        const newValue = !navbarOpen
        setNavbarOpen(newValue)
        setTransparent(!newValue && location.pathname === LINKS_DATA.main.to)
    }
    ...
    return (
        <>
            <nav className={"flex flex-wrap items-center justify-between px-2 py-4 navbar-expand-lg " + (transparent ? "bg-transparent top-0 absolute z-50 w-full" : "relative bg-white dark:bg-gray-800")}>
