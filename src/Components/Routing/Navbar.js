...
export default function Navbar() {
	...
	const toggleNavbar = () => {
		...
		setTransparentNavbar(
			!newValue && location.pathname === LINKS_DATA.main.to
		);
	};
		...
		return (
			<li className="nav-item group px-1" key={item.to}>
				<Link to={item.to} onClick={itemClicked}>
					<p
						className={
							"px-3 py-2 flex items-center text-md uppercase font-bold leading-snug hover:opacity-75 " +
							(transparentNavbar ? "text-white" : "text-gray-700")
						}
					>
						{item.title}
					</p>
	...
	return (
		<>
			<nav
				className={
					"flex flex-wrap items-center justify-between px-2 py-4 navbar-expand-lg " +
					(transparentNavbar
						? "bg-transparent top-0 absolute z-50 w-full"
				...
				<div className="container px-4 mx-auto flex flex-wrap items-center justify-between">
					<div className="w-full relative flex justify-between lg:w-auto lg:static lg:block lg:justify-start">
						<Link to="/" onClick={toHome}>
							<p
								className={
									"text-lg font-bold leading-relaxed inline-block px-3 py-2 whitespace-no-wrap uppercase " +
									(transparentNavbar
										? "text-white"
										: "text-black")
								}
